...
public class Hashids_<T where T:Equatable, T:UnsignedIntegerType> : HashidsGenerator
{
    ...
    public init(salt:String!, minHashLength:UInt = 0, alphabet:String? = nil)
    {
        ...
        self.seps = intersection(self.alphabet, b: self.seps);
        self.alphabet = difference(self.alphabet, b: self.seps);
    ...
    private func _decode(hash:[Char]) -> [Int]
    {
        ...
        var hashes = hash.split(hash.count, allowEmptySlices: true, isSeparator: { self.guards.contains($0)});
        ...
        if(hash.count > 0)
        {
            ...
            let valueHashes = valuesHashes.split(valuesHashes.count, allowEmptySlices: true, isSeparator: { self.seps.contains($0)});
...
{
    return transform(a, b: a) { (var c, a, b, e) in
        if !c.contains(e)
        {
            c.append(e);
        };
...
{
    return transform(a, b: b) { (var c, a, b, e) in
        if b.contains(e)
        {
            c.append(e);
        };
